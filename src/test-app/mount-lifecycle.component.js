...
import {registerApplication, triggerAppChange, getAppStatus, MOUNTED, getAppNames} from 'single-spa'
...
export default React.forwardRef(function MountLifecycle({app, stepNumber}, ref) {
  ...
  if (ref) {
    ref.current = {
      runTest() {
        ...
        /* global System */
        registerApplication(app.name, () => System.import(app.name), () => shouldMount)
        ...
        return triggerAppChange()
          .then(() => {
            if (getAppStatus(app.name) !== MOUNTED) {
              throw Error(`After attempting to mount the application '${app.name}', the app is in ${getAppStatus(app.name)} status instead of ${MOUNTED} status`)
            } 
          })
          .catch(err => {
            setError(err)
    ...
    <div className="step">
      <div role="button" tabIndex={0} className="step-header" onClick={toggleExpanded}>
        ...
        <div className="step-summary">
          Application can mount to the DOM
        </div>
      </div>
      {(isExpanded || error) &&
