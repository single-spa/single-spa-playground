        ...
        return triggerAppChange()
          .then(() => {
            if (getAppStatus(app.name) !== MOUNTED) {
              throw Error(`After attempting to remount the application '${app.name}', the app is in ${getAppStatus(app.name)} status instead of ${MOUNTED} status`)
            }
    ...
    <div className="step">
      <div role="button" tabIndex={0} className="step-header" onClick={toggleExpanded}>
        ...
        <div className="step-summary">
          Remounting the application works
        </div>
      </div>
      {(isExpanded || error) &&
