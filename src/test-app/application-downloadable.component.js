...
export default React.forwardRef(function ApplicationDownloadable({app, stepNumber}, ref) {
      ...
      .then(resp => {
        if (resp.ok) {
          ...
          if (!contentType.includes('application/javascript')) {
            throw Error(`Could not download app -- server didn't respond with javascript code, but with '${contentType}'`)
          }
        } else {
          throw Error(`Could not download app -- server responded with status '${resp.status}'`)
        }
      })
      .catch(err => {
        setError(err)
