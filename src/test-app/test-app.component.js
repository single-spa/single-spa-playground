  ...
  return (
    <>
      ...
      <article className="card" {...scope}>
        ...
        <section>
          Download url: <a href={window.importMapOverrides.getOverrideMap().imports[app.name]} target="_blank">{window.importMapOverrides.getOverrideMap().imports[app.name]}</a>
        </section>
        <section>
          {nextTestIndex >= stepComponents.length
            ...
            :
              <button className="primary" type="button" onClick={doTest} style={{visibility: restartingTests ? 'hidden' : 'visible'}}>
                {buttonText()}
              </button>
          }
        </section>
        ...
        <section>
          You can also verify if your application is working by navigating this browser tab to the url that it is in charge of. This will let you
          see your application working inside of this browser tab. The single-spa playground will disappear, and your code will take over.
        </section>
        <section>
          To do this, click on the navigate to application link. You can come back to the playground by clicking on the blue single-spa logo that will appear in the
          bottom right of your screen. Once you've verified things are working as expected, proceed to the next step.
        </section>
        <section>
          <a className="secondary button" href={app.pathPrefix} onClick={navigateToUrl}>
