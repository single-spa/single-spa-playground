...
export default React.forwardRef(function UnmountLifecycle({app, stepNumber}, ref) {
        ...
        return triggerAppChange()
          .then(() => {
            ...
            if (status !== NOT_MOUNTED) {
              throw Error(`Application '${app.name}' did not unmount correctly. Instead of being in status UNMOUNTED, it is in status ${status}`)
            }
          })
          .catch(err => {
            setError(err)
    ...
    <div className="step">
      <div role="button" tabIndex={0} className="step-header" onClick={toggleExpanded}>
        ...
        <div className="step-summary">
          Unmount lifecycle works correctly
        </div>
      </div>
      {(isExpanded || error) &&
