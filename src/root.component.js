...
export default function Root(props) {
  const [showPlayground, setShowPlayground] = useState(() => location.pathname.startsWith('/playground'))
  const localStorageData = useLocalStorageData()

  useEffect(() => {
    window.addEventListener('single-spa:routing-event', routeChange)
    return () => window.removeEventListener('single-spa:routing-event', routeChange)

    function routeChange()  {
      const shouldShowPlayground = window.location.pathname.startsWith('/playground')
      setShowPlayground(shouldShowPlayground)
    }
  }, [])

  useEffect(() => {
    if (!localStorage.getItem('single-spa-playground') && !window.location.pathname.startsWith('/playground')) {
      navigateToUrl('/playground')
    }
  }, [])

  useEffect(() => {
    Promise.all(getAppNames().map(appName => window.__SINGLE_SPA_DEVTOOLS__.exposedMethods.unregisterApplication(appName))).then(() => {
      if (!showPlayground) {
